...
const worker = new Worker('/dist/test-worker.bundle.js');
...
describe('jz.common', () => {
  ...
  describe('.readFileAsArrayBuffer(blob)', () => {
    it('reads the file as ArrayBuffer.', () => {
      return jz.common.readFileAsArrayBuffer(new Blob([new Uint8Array(100)])).then(buffer => {
  ...
  describe('.readFileAsText(blob, encoding)', () => {
    it('reads the file as string.', () => {
      return jz.common.readFileAsText(new Blob(['こんにちは世界'])).then(text => {
  ...
  describe('.readFileAsDataURL(blob)', () => {
    it('reads the file as data URL.', () => {
      return jz.common.readFileAsDataURL(new Blob(['hello'])).then(dataURL => {
  ...
  describe('.load(urls)', function() {
    it('loads files as Uint8Array.', function() {
      return jz.common.load(URL.createObjectURL(new Blob([new Uint8Array([1, 2, 3])]))).then(bytess => {
...
describe('jz.zlib', () => {
  ...
  before(() => {
    return jz.common.load('test/nicowari.swf').then(([_bytes]: any) => {
...
describe('jz.gz', () => {
  ...
  before(() => {
    return jz.common.load('test/sample.txt.gz').then(_gzStream => (gzStream = _gzStream[0]));
  });

  describe('.decompress({buffer, chunkSize})', () => {
    it('decompresses the gzip stream.', () => {
      expect(jz.gz.decompress({ buffer: gzStream })).to.be.instanceof(Uint8Array);
    });
  });

  describe('jz.gz.compress({buffer, level, chunkSize})', () => {
    it('compresses to the gzip stream.', () => {
      return Promise.resolve({ buffer: gzStream })
...
describe('jz.zip', () => {
  ...
  describe('ZipBufferArchiveReader', function() {
    ...
    describe('#getFileNames()', () => {
      it('gets file names in a zip archive.', () => {
        expect(reader.getFileNames()).to.eql(['zipsample/a.txt', 'zipsample/folder/b.txt']);
      });
    });

    describe('#readFileAsText(filename, encoding)', () => {
      it('reads the file as text.', () => {
        return reader.readFileAsText('zipsample/a.txt').then(a => {
          expect(a).to.equal('a');
        });
      });
    });
    ...
    describe('#readFileAsBlob(filename)', () => {
      it('reads the file as Blob.', () => {
        return reader.readFileAsBlob('zipsample/a.txt').then(a => {
    ...
    describe('#readFileAsArrayBuffer(filename)', () => {
      it('reads the file as ArrayBuffer.', () => {
        return reader.readFileAsArrayBuffer('zipsample/a.txt').then(a => {
    ...
    describe('#readFileAsDataURL(filname)', () => {
      it('reads the file as data URL.', () => {
        return reader.readFileAsDataURL('zipsample/a.txt').then(a => {
  ...
  describe('ZipBlobArchiveReader', () => {
    ...
    before(() => {
      return jz.common
        .load('/test/zipsample.zip')
        .then(([buffer]: any) => jz.zip.unpack({ buffer: new Blob([buffer]) }))
        .then(_reader => (reader = _reader));
    });
      ...
      const files = [
        {
          ...
          dir: [
            ...
            {
              name: 'folder',
              dir: [{ name: 'b.txt', url: '/test/zipsample/folder/b.txt' }],
            },
          ],
        },
      ];
